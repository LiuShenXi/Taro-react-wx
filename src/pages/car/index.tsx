...
const Car: React.FC = () => {
  ...
  const [items, setItems] = useState<BeautyService[]>(
    beautyServices.map((item) => ({ ...item, isSelected: false }))
  );
  ...
  useEffect(() => {
    ...
    const total = items.reduce(
      (sum, item) =>
        item.isSelected ? sum + item.num * parseFloat(item.price) : sum,
      0
    );
    setTotalPrice(total);
  }, [items]);
  ...
  const handleItemSelect = (title, selected) => {
    setItems((prev) =>
      prev.map((item) =>
        item.title === title ? { ...item, isSelected: selected } : item
      )
    );
  };
  ...
  const handleItemCountChange = (title, num) => {
    setItems((prev) =>
      prev.map((item) =>
        item.title === title ? { ...item, num: parseInt(num, 10) } : item
      )
    );
  };
